...
def mafft():
    ...
    fasta_for_phylo_path = "/var/www/cgi-bin/phylophile/" +  result_dir + project_name + "-blast_hits_added.fasta"
    mafft_aln = result_dir + project_name + "-mafft.aln"
    mafft_cmd = "mafft --quiet --localpair --maxiterate 1000 --nuc --thread " + cpus_avail + " " + fasta_for_phylo_path + " > " + mafft_aln
        
    os.popen(mafft_cmd)
    tmp_log_file.write("\n")
    tmp_log_file.write("mafft_aln=")
    tmp_log_file.write(mafft_aln)
    tmp_log_file.write("\n")
    tmp_log_file.write(mafft_cmd)
...
if __name__ == "__main__":
        try:
           ...
           #print information out to the webpage
            ...
            '''
            #time the fuction mafft
            #t=Timer(mafft)
            #mafftTime = (t.timeit(1))
            #projectName = log_dict["project_name"]
            #line = projectName +  ",mafftTime," + str(mafftTime) + "\n"
            ...
            #now redirect to trimal
            target_url = '<meta http-equiv="refresh" content="0;url=http://' + ip + '/cgi-bin/phylophile/trimal.py" /> '
            print target_url
